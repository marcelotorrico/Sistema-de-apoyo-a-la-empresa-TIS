...
$GLOBALS['CONFIG']['DISPLAY_HIDDEN'] = TRUE;
...
$GLOBALS['CONFIG']['ENABLE_IMAGE_ICONS'] = TRUE;
...
$GLOBALS['CONFIG']['CODE_FILES'] = array('php','php4','php3','phtml','html','htm','js','asp','xml','css','bml','cgi','cfm','apm','jhtml','xhtml','aspx','tpl','inc','c','h','vb','py','sh','pl','cpp','cs','sql','txt');
...
elseif (preg_match('#[.]/#',$dir)) $dir = preg_replace('#[.]/#','',$dir);
elseif (!preg_match("#$basedir#i",$dir)) $dir = '';
elseif (!$dir) $dir = '';
elseif ($dir == '.') $dir = '';
while (preg_match('#//#',$dir)) $dir = preg_replace('#//#','/',$dir);
while (preg_match('#//#',$file)) $file = preg_replace('#//#','/',$file);
if ($dir && $dir{strlen($dir)-1} == '/') $dir{strlen($dir)-1} = '';
if ($dir && $dir{0} != '/') $dir = '/'.$dir;
if ($dir && $dir{0} == '/') $dirlink = substr($dir,1).'/';
...
if (isprotecteddir($dir) && (!isset($_GET['p']) || $_GET['p'] != 'logo')) {
...
function listdir($directory) {
		...
		if (($GLOBALS['CONFIG']['DISPLAY_HIDDEN'] || (!$GLOBALS['CONFIG']['DISPLAY_HIDDEN'] && $file{0} != ".")) 
		 	...
		 	if (is_dir($basedir.$directory.'/'.$file)) {
		 		$dirc[] = '<td width="100%"><table cellpadding="0" cellspacing="0" width="100%"><tr><td width="18">'.
		 		 	 	 	 '<img src="'.$_SERVER['SCRIPT_NAME'].'?p=mime&amp;type='.getmime($directory.'/'.$file).'" border="0" alt=""><td align="left"><a href="'.$_SERVER['SCRIPT_NAME'].'?dir='.$directory.'/'.$file.'">'.$file.'</a></table>'.
	...
	echo '<tr><td colspan="3" nowrap><b>'.$GLOBALS['STRINGS']['CURRENT_DIRECTORY'].': ';
        $t = 1; //para solucionar el problema de que no está definida
        $dirlist[0] = '';
		 	...
		 	if ($file != '.' && $file != '..' && !is_dir($file)) { 
		 	 	...
		 	 	if ($GLOBALS['CONFIG']['ENABLE_LINE_COUNT']) {
			 	 	if(iscodetype($file)) {
			 	 		$lines = file($dir.'/'.$file);
			 	 		$linecount = 0;
			 	 		foreach ($lines as $line) if (substr(preg_replace('# #','',$line),0,2) != '//'|'/*') $linecount++;
...
function pagehead($stitle) {
	...
	echo 	'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">'.
			...
			'<link rel="shortcut icon" href="'.$_SERVER['SCRIPT_NAME'].'?p=favicon">'.
	 		...
	 		'<tr><td align="left" width="700" bgcolor="'.$GLOBALS['CONFIG']['TB_COLOR_4'].'"><img src="'.$GLOBALS['CONFIG']['BG_IMAGE'].'" alt="'.$dtitle.'"> '.$dtitle.
...
function url_encode($url) {
	return preg_replace('#[+]#i','%20',urlencode($url));
}
