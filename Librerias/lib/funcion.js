...
function agregarFila(num) {
    ...
    idTabla = "#tabla_" + num;
    fila = '<tr>' +
            '<td><input type="text" maxlength="150" style="width: 75%;" required name="nombre"></td>' +
            '<td><input type="number" min="0" max="100" required name="nota"></td>' +
            '</tr>';
    $(idTabla + " tr:last").after(fila);
    $(idTabla).show();
    //$(document).load("../Vista/evaluacion.php");
}
function mostrarResultado(tamano) {
    var tabla, nombres, notas, suma, arregloNombre, arregloNota, arregloNotaFinal, tam, lim;
    arregloNombre = "";
    arregloNota = "";
    arregloNotaFinal = new Array();
    tabla = "#tabla_";
    lim = tamano - 1;
    for (var i = 0; i < tamano; i++) {
        ...
        notas = new Array();
        suma = 0;
        $(idTabla + " input[type='text']").each(function () {
            var valor = $(this).val();
            if (valor != "") {
                nombres.push(valor);
            }
        });
        $(idTabla + " input[type='number']").each(function () {
        ...
        tam = $(idTabla + " input[type='number']").length;
        $("#nota" + i).val(suma / tam);
        arregloNotaFinal[i] = $("#nota" + i).val();
        if (nombres.length == notas.length) {
            arregloNombre += nombres;
            arregloNota += notas;
            if(i<lim){
                arregloNombre+="|";
                arregloNota+="|";
        ...
        window.location.href = 'evaluacionFinal.php?arregloNombre='+arregloNombre+'&arregloNota='+arregloNota+'&arregloNotaFinal='+arregloNotaFinal;
}
